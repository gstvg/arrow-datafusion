                            ...
                            std::iter::zip(&partially_planned, &mut fields)
                        {
                            match (arg, field) {
                                (
                                    ...
                                    let params = params.next().expect(
                                        "params len should have been checked above",
                                    );
                                    if let Some(params) = params {
                                        // todo: cache this when the params doesn't change between iterations
                                        // with something like ExprOrLambda::Lambda(Option<datafusion_expr::Lambda, Vec<Field>>, ast::LambdaFunction)
                                ...
                                (ExprOrLambda::Expr(_), ValueOrLambda::Value(_)) => {}
